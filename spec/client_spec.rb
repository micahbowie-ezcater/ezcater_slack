...
RSpec.describe EzcaterSlack::Client do
  after do
  ...
  describe '#configuration' do
    it 'creates a new instance of Configuration' do
      expect(described_class.configuration).to be_instance_of(EzcaterSlack::Configuration)
    end
  end
  ...
  describe '#version' do
    it 'has a version number' do
      expect(EzcaterSlack::VERSION).not_to be nil
    end
  end
  ...
  describe '#send_message' do
    ...
    let(:compilation) do
      instance_double("EzcaterSlack::Message::Compilation", 
                      channel: 'C1234567890', 
                      blocks: [], 
                      csv?: false)
    end
    ...
    before do
      allow(EzcaterSlack::Message).to receive(:compile).and_return(compilation)
    ...
    it 'compiles the message and sends it to Slack' do
      expect(EzcaterSlack::Message).to receive(:compile).with(template_name, args)
    ...
    context 'when the compilation includes csv data' do
      ...
      it 'uploads a CSV file along with the message' do
        allow(EzcaterSlack::Message).to receive(:compile).and_return(compilation_with_csv)
    ...
    context 'when Slack::Web::Api::Errors::SlackError is raised' do
      it 'rescues from Slack::Web::Api::Errors::SlackError' do
        allow(EzcaterSlack::Message).to receive(:compile).and_raise(Slack::Web::Api::Errors::SlackError.new('Error'))
